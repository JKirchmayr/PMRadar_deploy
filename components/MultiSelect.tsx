...
export function MultiSelect({
  ...
  return (
    <>
      <Command onKeyDown={handleKeyDown} className="overflow-visible bg-transparent outline-none">
        <div className="group rounded-sm border bg-white border-gray-200 px-2 py-1 text-xs">
        ...
        <div className="relative mt-1.5 bg-white">
          <CommandList>
            {open && selectables.length > 0 ? (
              <div className="absolute top-0 z-50 w-full rounded-sm border bg-popover text-popover-foreground shadow-md outline-none animate-in">
      ...
      >
        {selectedOptions.map((option) => (
          <Badge
            key={option.value}
            variant="secondary"
            className="text-[10px] text-gray-700 border border-gray-200 font-normal"
          >
            {option.label}
            <button
              className="ml-1 text-[10px] rounded-full outline-none"
